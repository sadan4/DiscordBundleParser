...
import { hasExportModifier, isAmbientDeclaration } from "../util";
...
export class UsageWalker {
            ...
            switch (node.kind) {
                ...
                case ts.SyntaxKind.ExportAssignment:
                    if (
                        ...
                        return this.#scope.markExported((node as ts.ExportAssignment).expression as ts.Identifier);
                    }
                ...
                case ts.SyntaxKind.Identifier: {
                    ...
                    if (domain !== undefined) {
                        this.#scope.addUse({
                            domain,
                            location: node as ts.Identifier,
                        });
                    }
                ...
                case ts.SyntaxKind.Parameter:
                    if (
                        ...
                        && ((node as ts.ParameterDeclaration).name.kind !== ts.SyntaxKind.Identifier
                          // eslint-disable-next-line @stylistic/max-len
                          || identifierToKeywordKind((node as ts.NamedDeclaration).name as ts.Identifier) !== ts.SyntaxKind.ThisKeyword)
                    ) {
    ...
    #handleModule(
        ...
        if (node.name.kind === ts.SyntaxKind.Identifier) {
            ...
            const ambient = isAmbientDeclaration(node);
    ...
    #handleVariableDeclaration(declarationList: ts.VariableDeclarationList) {
        ...
        const exported
            = declarationList.parent.kind === ts.SyntaxKind.VariableStatement
              && hasExportModifier(declarationList.parent);
